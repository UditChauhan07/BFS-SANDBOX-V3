      ...
      data?.map((ele) => {
        if (ele.content.length) {
          ele.content.map((item) => {
            ...
            temp["Product Ship Date"] = product.Ship_Date__c ? (item.Ship_Date__c.split("-")[2] == 15 ? 'TBD' : item.Ship_Date__c.split("-")[2]) + '/' + monthNames[parseInt(item.Ship_Date__c.split("-")[1]) - 1].toUpperCase() + '/' + item.Ship_Date__c.split("-")[0] : 'NA';
