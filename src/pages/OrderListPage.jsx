...
const OrderListPage = () => {
  ...
  const orderData = useMemo(() => {
      return orders
          ...
          ?.filter((order) => 
              ...
              order.AccountName?.toLowerCase().includes(filterValue.search.toLowerCase())
          )
          .filter((order) => {
              if (searchShipBy) {
                  ...
                  return (
                      orderItems?.some((item) => 
                          item.Name?.toLowerCase().includes(searchShipBy.toLowerCase())
                      ) ||
  ...
  const getOrderlIsthandler = ({ key, Sales_Rep__c }) => {
      getOrderCustomerSupport({
          ...
          .then((order) => {
              setOrders(order.sort((a, b) => new Date(b.CreatedDate) - new Date(a.CreatedDate)));
              setLoaded(true);
          })
  ...
  return (
      <AppLayout // Use AppLayout if appropriate, or replace with CustomerSupportLayout
          filterNodes={
              <>
