...
import { BiLeftArrow, BiLock, BiRightArrow } from "react-icons/bi";

const OrderCardHandler = ({ orders, setOrderId, orderId, reason, orderConfirmedStatus, files = [], desc,errorListObj,manufacturerIdObj,accountIdObj }) => {
                    ...
                    if (((searchPo && searchPo != "") ? item.PO_Number__c?.toLowerCase().includes(
                        ...
                        return (
                            <div className={` ${Styles.orderStatement} cardHover ${orderId == item.Id ? Styles1.selOrder : ''}`} style={{ paddingBottom: '15px' }} key={index}>
                                        ...
                                        <div className={Styles1.totalProductPrice}>
                                            {(orderId && (!searchPo || searchPo == "")) && <>
                                                ...
                                                <div className={Styles1.Margitotal}>
                                                    ...
                                                    <p className={Styles1.detailsDescHolder}>Customer Service for {reason} having <br /> PO# {item.PO_Number__c} Created on {datemonth}</p>
                                                </div>
                                            </>}
                                            <div className={Styles1.Margitotal}>
                                            ...
                                            {(orderId && (!searchPo || searchPo == "")) && <>
                                                ...
                                                {(orderId == item.Id && !orderConfirmed) && <div className={Styles1.Margitotal}>
                                                    <button className={Styles1.btnHolder} onClick={() => orderConfirmationHandler()}>Next <BiRightArrow /></button>
                                                </div>}
                                                {(orderId == item.Id && orderConfirmed) && <div className={Styles1.Margitotal}>
                                                    <button className={Styles1.btnHolder} onClick={() => setOrderConfirmed(false)}><BiLeftArrow />Reset</button>
                                                </div>}
                                ...
                                {orderId && <b onClick={() => { resetForm() }} style={{ cursor: 'pointer', marginLeft: '15px', textDecoration: 'underline' }}>Wrong Order? Re-Select Order again.</b>}
                            </div>
                        )
                    }
                })}
