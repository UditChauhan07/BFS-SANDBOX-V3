...
const MobileHeader = () => {
  ...
  <path d="M7.08301 24.0835H15.583" stroke="black" strokeWidth="2" stroke-linecap="round"/>
        ...
        <div className={styles.ControlMobNav}>
          <p className={`m-0 pt-2 ${styles.myRetailer}`}>
            <Link to="/order" className={`${styles.myRetailer}`}>
           Order Now
            </Link>
          </p>
          <p className={`m-0  pt-2 ${styles.myRetailer} `}>
          ...
          {/* <div class="dropdown">
            <button className={styles.dropdownButton} onClick={toggleDropdown} type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
            <p className={`m-0  pt-2 ${styles.myRetailer}`}>
              <Link to="" className={`${styles.myRetailer}`}>
                <div className={styles.MoblabelLine}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="12" height="10" viewBox="0 0 12 10" fill="none">
            ...
            {dropdownVisible && (
              <div className={styles.dropdownMenu}>
                   <ul class="dropdown-menu ms-5 mt-1"  aria-labelledby="dropdownMenuButton1">
                {currentFilters.length > 0 ? (
                  currentFilters.map((filter, index) => (
                 
                    <li><a key={index} className={styles.dropdownItem}>{filter}</a></li>
                  
                  ))
                ) : (
                  <li className={styles.dropdownItem}>No filters available</li>
                )}
                  </ul> 
              </div>
            )}
