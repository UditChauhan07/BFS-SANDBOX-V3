  ...
  return (
    <div className="container mt-4 product-card-element">
      <div className="d-flex">
        <div className={`${Styles.ProductImage} col-4 gap-4`} id={`${Styles.ProductImage}`}>
          {product?.data?.imgSrc.length > 0 ? <Slider data={product?.data?.imgSrc} /> : <Slider data={fakeProductSlider} />}
        </div>
        <div className={`${Styles.productCardholder} col-8 ml-4 product-card-element-holder`}>
          ...
          {product?.discount ? (
            ...
            <p style={{ textAlign: "start" }}><b>{product?.data?.usdRetail__c}</b></p>
          )}
          {product?.data?.Description && <p style={{ textAlign: "start", color:"#898989"}}>{product?.data?.Description}</p>}
          
       <hr style={{ borderTop: "1px dashed", color: "black",fontSize:"25px" }} ></hr>
          <p style={{ textAlign: "start", color:"#898989" ,}}>Product Code: <b className={`${Styles.spancontent}`} >{product?.data?.ProductCode}</b></p>
          <p style={{ textAlign: "start",color:"#898989" }}>Product UPC: <b className={`${Styles.spancontent}`}>{product?.data?.ProductUPC__c}</b>,</p>
          <p style={{ textAlign: 'start',color:"#898989" }}>Min Order QTY: <b className={`${Styles.spancontent}`}>{product?.data?.Min_Order_QTY__c}</b></p>
            {product?.data?.Category__c && <p style={{ textAlign: "start",color:"#898989" }}>Category: <b className={`${Styles.spancontent}`}>{product?.data?.Category__c}</b></p>}
          {product.data?.Collection__c && <p style={{ textAlign: "start",color:"#898989" }}>Collection: ,<b className={`${Styles.spancontent}`}>{product.data?.Collection__c}</b></p>}
